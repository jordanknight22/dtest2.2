...
def quote_data():
    ...
    df_quotes["no_of_pets"] = df_quotes["SourceFile"].map(df_quotes["SourceFile"].value_counts())
    ...
    df_quotes["pet_age_mnths"] = df_quotes.apply(
        lambda x: (
            pd.to_datetime(x["CoverStartDateTime"]).year - pd.to_datetime(x["PetDOB"]).year
        ) * 12
        + (pd.to_datetime(x["CoverStartDateTime"]).month - pd.to_datetime(x["PetDOB"]).month),
        axis=1
    )
    ...
    # Pet Type
    df_quotes = df_quotes.rename(columns={"PetType": "pet_type"})
    df_quotes["pet_type"] = df_quotes["pet_type"].str.lower()

    # Fill dog if petsubtype is pedigree or crossbreed
    df_quotes.loc[
        df_quotes["pet_type"].isna() & df_quotes["PetSubType"].str.lower().isin(["pedigree", "crossbreed"]),
        "pet_type"
    ] = "dog"

    # Fill cat if petsubtype is moggie
    df_quotes.loc[
        df_quotes["pet_type"].isna() & (df_quotes["PetSubType"].str.lower() == "moggie"),
        "pet_type"
    ] = "cat"
